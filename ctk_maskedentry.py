...
class CTkMaskedEntry(CTkBaseClass):
    ...
    def _start_mask(self):
        ...
        self._textvariable.trace_add('read', self._variable_changed)

    def _variable_changed(self, string1: str, string2: str, string3: str):
        if not self._have_mask:
            assert 'A CTkMaskedEntry method was runned and will be ignored.'
            return 'break'
        value = self._textvariable.get()
        formated_value = self._mask.format_variable(value)
        self._textvariable.set(formated_value)
    ...
    def _onkeypress(self, event):
        ...
        if event.keysym == 'Left':
            ...
            def repl_or_stop(cls, pos):
                __mask = cls._mask
                if 0 <= pos <= __mask.cget('len'):
                    if not __mask.cget('tests')[pos]:
                        pos = __mask.seekprev(pos)
                    cls._write_char(pos, __mask.cget('placeholder'), -1)
                return 'break'

            repl_or_stop(self, widget.index(tkinter.INSERT)-1)
            return 'break'
        else:
            if self._mask.cget('type') == 'fixed':
